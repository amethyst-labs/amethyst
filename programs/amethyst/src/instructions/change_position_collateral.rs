...
#[derive(Accounts)]
pub struct ChangePositionCollateral<'info> {
    /// The vault's cache.
    #[account(
        mut,
        has_one = vault
...
impl<'info> ChangePositionCollateral<'info> {
    /// Perform validation.
    ///
    ///
    ...
    /// Change the position's collateral.
    fn change_position_collateral(&self, value_change: PositionValueChange) -> Result<()> {
        match value_change {
            PositionValueChange::Increase(amount) => self.deposit_collateral(amount),
            PositionValueChange::Decrease(amount) => self.withdraw_collateral(amount),
        }
    }
...
pub fn handler(
    ...
    ctx.accounts.change_position_collateral(value_change)?;
    ctx.accounts.post_validation()?;
    Ok(())
}

impl_change_position_ctx! { ChangePositionCollateral<'info> }
